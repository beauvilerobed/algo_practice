...
class MergeArray(unittest.TestCase):
    ...
    def test_large(self):
        test_cases = [
            ([1 for i in range(100)], [0 for i in range(1000)], [0 for i in range(1000)]+[1 for i in range(100)]),
            ([1] * 10 ** 5, [7], [1] * 10 ** 5 + [7])
        ]
        for nums1, nums2, answer in test_cases:
            self.assertEqual(merge_array(nums1, nums2), answer)
    ...
    def test_stress(self):
        ...
        for _ in range(10000):
            temp = [random.randint(1, 1000) for _ in range(random.randint(50, 100))]
            temp.sort()
        ...
        for i in range(0, 1000, 2):
            ...
            self.assertEqual(merge_array(nums1, nums2), merge_array_naive(nums1, nums2))
